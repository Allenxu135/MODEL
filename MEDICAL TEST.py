...
class MedicalConfig:
    def __init__(self):
        ...
        # 验证审核机制配置
        self.validation_threshold = 0.95  # 验证阈值
        self.high_confidence_threshold = 0.97  # 高置信度阈值，超过此值跳过验证
        self.validation_depth = 2  # 验证思考深度
        ...
        logger.info(f"验证阈值 | Validation Threshold: {self.validation_threshold * 100}%")
        logger.info(f"高置信度阈值 | High Confidence Threshold: {self.high_confidence_threshold * 100}%")
        logger.info("===================================================")
...
class ValidationMechanism:
    ...
    async def validate_diagnosis(self, diagnosis_result, symptoms):
        """验证诊断结果的准确性和权威性"""
        ...
        if confidence >= self.config.high_confidence_threshold:
            validation_log["steps"].append({
                ...
                "result": f"置信度 {confidence:.3f} 超过高阈值 {self.config.high_confidence_threshold}，跳过验证",
                "confidence_impact": 0.0
            })
            validation_log["validated"] = True
            self.validation_results.append(validation_log)
            return True, validation_log

        # 步骤2: 深度知识库验证
        validation_log["steps"].append({
            "step": "knowledge_base_validation",
            "result": "开始知识库深度验证",
            "confidence_impact": 0.0
        })

        # 使用知识图谱进行深度验证
        kb_validation_score = await self.validate_with_knowledge_graph(disease, symptoms)
        new_confidence = confidence * (0.7 + 0.3 * kb_validation_score)

        validation_log["steps"].append({
            "step": "knowledge_graph_validation",
            "result": f"知识图谱验证得分: {kb_validation_score:.3f}",
            "confidence_impact": new_confidence - confidence
        })
        confidence = new_confidence

        # 步骤3: 症状匹配度验证
        symptom_match_score = await self.validate_symptom_match(disease, symptoms)
        new_confidence = confidence * (0.6 + 0.4 * symptom_match_score)

        validation_log["steps"].append({
            "step": "symptom_match_validation",
            "result": f"症状匹配度得分: {symptom_match_score:.3f}",
            "confidence_impact": new_confidence - confidence
        })
        confidence = new_confidence

        # 步骤4: 权威来源验证
        authority_score = await self.validate_with_authoritative_sources(disease)
        new_confidence = confidence * (0.8 + 0.2 * authority_score)

        validation_log["steps"].append({
            "step": "authority_validation",
            "result": f"权威来源验证得分: {authority_score:.3f}",
            "confidence_impact": new_confidence - confidence
        })
        confidence = new_confidence

        # 最终验证结果
        validation_log["final_confidence"] = confidence
        validation_log["validated"] = confidence >= self.config.validation_threshold

        validation_log["steps"].append({
            "step": "final_validation",
            "result": f"最终置信度: {confidence:.3f}, 验证结果: {'通过' if validation_log['validated'] else '未通过'}",
            "confidence_impact": 0.0
        })

        self.validation_results.append(validation_log)
        return validation_log["validated"], validation_log

    async def validate_with_knowledge_graph(self, disease, symptoms):
        """使用知识图谱进行深度验证"""
        try:
            # 查找疾病实体
            disease_entities = self.knowledge_base.knowledge_graph.find_entities(disease, "disease")
            if not disease_entities:
                return 0.5  # 中等分数，表示不确定

            # 获取疾病的相关症状
            related_symptoms = set()
            for entity_id, entity_data in disease_entities:
                related_entities = self.knowledge_base.knowledge_graph.find_related_entities(
                    entity_id, "has_symptom", max_depth=1
                )
                for related_id, related_data in related_entities:
                    if related_data.get('type') == 'symptom' and 'name' in related_data:
                        related_symptoms.add(related_data['name'])
            ...
            for symptom in symptoms:
                ...
                for related_symptom in related_symptoms:
                    related_symptom_en = await self.config.translate_to_english(
                        related_symptom) or related_symptom.lower()
                    similarity = 1 - (Levenshtein.distance(symptom_en, related_symptom_en) /
                                      max(len(symptom_en), len(related_symptom_en)))
                    if similarity > 0.7:
                        symptom_match_count += 1
                        break

            match_score = symptom_match_count / len(symptoms) if symptoms else 0
            return 0.3 + 0.7 * match_score  # 基础分0.3，最高1.0

        except Exception as e:
            logger.error(f"知识图谱验证错误: {str(e)}")
            return 0.5

    async def validate_symptom_match(self, disease, symptoms):
        """验证症状与疾病的匹配度"""
        try:
            # 获取疾病的标准症状
            disease_info = self.knowledge_base.disease_info.get(disease, {})
            standard_symptoms = disease_info.get("symptoms", [])

            if not standard_symptoms:
                return 0.6  # 中等分数
            ...
            for symptom in symptoms:
                symptom_en = await self.config.translate_to_english(symptom) or symptom.lower()
                for std_symptom in standard_symptoms:
                    std_symptom_en = await self.config.translate_to_english(std_symptom) or std_symptom.lower()
                    similarity = 1 - (Levenshtein.distance(symptom_en, std_symptom_en) /
                                      max(len(symptom_en), len(std_symptom_en)))
                    if similarity > 0.7:
                        matched_symptoms += 1
                        break

            match_score = matched_symptoms / len(symptoms) if symptoms else 0
            return 0.4 + 0.6 * match_score  # 基础分0.4，最高1.0

        except Exception as e:
            logger.error(f"症状匹配验证错误: {str(e)}")
            return 0.5

    async def validate_with_authoritative_sources(self, disease):
        """使用权威来源验证疾病"""
        try:
            # 检查知识库中是否有该疾病的权威信息
            disease_info = self.knowledge_base.disease_info.get(disease, {})
...
class MedicalAssistant:
    def __init__(self, knowledge_base, config, model, tokenizer, label_map):
        ...
        self.validation_mechanism = ValidationMechanism(knowledge_base, config)  # 验证审核机制
    ...
    async def diagnose(self, chief_complaint):
        ...
        # 步骤3: 验证审核机制
        validation_passed, validation_log = await self.validation_mechanism.validate_diagnosis(diagnosis,
                                                                                               final_symptoms)

        # 记录验证过程
        for step in validation_log["steps"]:
            self.thought_process.append(f"验证步骤 | Validation step: {step['step']} - {step['result']}")

        self.thought_process.append(f"验证结果 | Validation result: {'通过' if validation_passed else '未通过'}")
        self.thought_process.append(f"最终置信度 | Final confidence: {validation_log['final_confidence'] * 100:.1f}%")

        # 根据验证结果决定是否继续
        if not validation_passed and validation_log["final_confidence"] < self.config.diagnosis_threshold:
            self.thought_process.append(
                f"置信度低于阈值 {self.config.diagnosis_threshold * 100}%，请求更多信息 | Confidence below threshold, requesting more information")
            return await self.request_more_info(chief_complaint, validation_log["final_confidence"])

        # 更新诊断置信度为验证后的值
        diagnosis["confidence"] = validation_log["final_confidence"]
    ...
    async def generate_final_response(self, diagnosis, kb_match, medication, tests, validation_log):
        ...
        cn_response += f"验证结果: {'通过' if validation_log['validated'] else '未通过'}\n\n"
        ...
        en_response += f"Validation Result: {'Passed' if validation_log['validated'] else 'Failed'}\n\n"
        ...
        for i, step in enumerate(validation_log["steps"]):
            cn_response += f"步骤 {i + 1}: {step['result']}\n"
            en_response += f"Step {i + 1}: {step['result']}\n"
