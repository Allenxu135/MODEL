...
import ollama
from langchain_community.document_loaders import TextLoader, CSVLoader, UnstructuredFileLoader
...
APP_STRINGS = {
    ...
    "install_deps": "Please install required dependencies: pip install 'unstructured[pdf,docx]' python-magic-bin"
}
...
class KnowledgeBase:
    ...
    def _load_file(self, file_path):
        """Load single file with detailed error handling"""
        ...
        try:
            ...
            if file_path.lower().endswith('.txt'):
                ...
                print(f"Using unstructured loader for: {file_path}")
    ...
    def import_data(self, file_paths):
        ...
        if not documents:
            ...
            return 0, "No valid documents found. " + APP_STRINGS["install_deps"]

        print(f"Total documents loaded: {len(documents)} from {valid_files}/{len(file_paths)} files")
        ...
        if not chunks:
            ...
            return 0, "No text chunks created"

        print(f"Split documents into {len(chunks)} text chunks")
        self.text_chunks = [chunk.page_content for chunk in chunks]

        # Try to create vector store with default model
        try:
            print(f"Creating vector store with {self.embedding_model}...")
            embeddings = OllamaEmbeddings(model=self.embedding_model)
            self.vector_store = FAISS.from_documents(chunks, embeddings)
            print("Vector store created successfully")
            return len(chunks), f"Success: {len(chunks)} chunks using {self.embedding_model}"
        except Exception as e:
            print(f"Error with {self.embedding_model}: {str(e)}")
            # Fallback to all-minilm if default fails
            try:
                print("Trying fallback embedding model: all-minilm")
                self.embedding_model = "all-minilm"
                embeddings = OllamaEmbeddings(model=self.embedding_model)
                self.vector_store = FAISS.from_documents(chunks, embeddings)
                print("Vector store created with fallback model")
                return len(chunks), f"Success: {len(chunks)} chunks using {self.embedding_model} (fallback)"
            except Exception as e2:
                print(f"Error with fallback model: {str(e2)}")
                return 0, f"Failed to create vector store: {str(e2)}. " + APP_STRINGS["install_deps"]
...
class DeepLearningGUI(QMainWindow):
    ...
    def import_knowledge(self):
        file_paths, _ = QFileDialog.getOpenFileNames(
            ...
            "All Files (*.*);;Text Files (*.txt);;CSV Files (*.csv);;PDF Files (*.pdf);;Word Documents (*.docx)"
        )

        if file_paths:
            try:
                chunk_count, result = self.knowledge_base.import_data(file_paths)
                if chunk_count > 0:
                    self.kb_status.setText(f"Success: {chunk_count} chunks imported")
                    self.kb_status.setStyleSheet("color: green;")
                else:
                    self.kb_status.setText(f"Import failed: {result}")
                    self.kb_status.setStyleSheet("color: red;")
            except Exception as e:
                self.kb_status.setText(f"Critical error: {str(e)}")
                self.kb_status.setStyleSheet("color: red;")
