...
class DialogueDataset(Dataset):
    def __init__(self, knowledge_base, config):
        ...
        # 简化: 直接从知识块创建简单训练样本
        print(f"Creating training examples from {len(knowledge_base.chunks)} chunks...")
        ...
        with open("training_examples.json", "w") as f:
            json.dump(self.examples[:10], f, indent=2)
...
class DialogueEngine:
    def __init__(self, model_path, config, knowledge_base):
        ...
        if not os.path.exists(model_path):
            ...
            # 尝试加载标准名称的模型
            final_model_path = os.path.join(os.path.dirname(model_path), "final_model.pth")
            if os.path.exists(final_model_path):
                print(f"Loading alternative model: {final_model_path}")
                model_path = final_model_path
            else:
                raise FileNotFoundError(f"No model found at {model_path} or {final_model_path}")
